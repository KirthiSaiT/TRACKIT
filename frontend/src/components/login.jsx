    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-950 via-purple-900 to-violet-950">
      <div className="w-full max-w-md p-8">
        <Card className="backdrop-blur-lg bg-white/10 border border-white/20 shadow-2xl">
          ...
          <CardContent className="space-y-6">
            <Button className="w-full bg-white text-black hover:bg-gray-200" onClick={signInWithGoogle}>
              Sign in with Google
            </Button>
            {!isLogin && (
              <div className="space-y-2">
                <Label className="text-white">Name</Label>
                <Input className="bg-gray-200" value={name} onChange={(e) => setName(e.target.value)} />
              </div>
            )}
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input className="bg-gray-200" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
            </div>
          </CardContent>
          <CardFooter className="flex flex-col space-y-4">
            <Button className="w-full bg-violet-600 hover:bg-violet-700 text-white" onClick={isLogin ? loginUser : registerUser}>
              {isLogin ? "Sign In" : "Sign Up"}
            </Button>
            <Button className="text-white" variant="ghost" onClick={() => setIsLogin(!isLogin)}>
              {isLogin ? "Don't have an account? Sign up" : "Already have an account? Sign in"}
            </Button>
          </CardFooter>
        </Card>
